...
fit_models <- function(data) {
  models <- list(
    full = lmer(y ~ 0+x1 + x2 + x3 + (1 + time + I(time^2) || group_id), data = data),
    reduced1 = lmer(y ~0+ x1 + x2 + x3 + (1 | group_id), data = data),
    reduced2 = lmer(y ~ 0+x1 + x2 + x3 + (1 + time || group_id), data = data)
  )
  return(models)
}
