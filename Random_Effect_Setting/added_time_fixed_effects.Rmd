...
fit_models <- function(data) {
  models <- list(
    full = lmer(y ~ time + I(time^2) + x1 + x2 + x3  + (1 + time + I(time^2) || group_id), data = data),
    reduced1 = lmer(y ~ time + I(time^2) + x1 + x2 + x3  + (1 | group_id), data = data),
    reduced2 = lmer(y ~ time + I(time^2) + x1 + x2 + x3  + (1 + time || group_id), data = data)
  )
  return(models)
}
...
results %>% bind_rows(.id = "m") %>% group_by(m) %>% summarise(mean(correct_model_normal), mean(correct_model_fitzmaurice), mean(correct_model_hybrid))
