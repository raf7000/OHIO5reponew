...
bic_fitzmaurice <- function(model) {
  ...
  n_params <- length(fixef(model)) + as.numeric(model@devcomp$dims["reTrms"]) + 1 
...
bic_normal <- function(model) {
  logLik_val <- as.numeric(logLik(model))
  n_params <- length(fixef(model)) + as.numeric(model@devcomp$dims["reTrms"]) + 1 #residual standard deviation parameter
...
bic_hybrid <- function(model) {
  logLik_val <- as.numeric(logLik(model))
  n_fixed <- length(fixef(model))+1
  n_random <- as.numeric(model@devcomp$dims["reTrms"])
  N <- nrow(model@frame) # Total number of observations
...
generate_data <- function(ni, m, beta, random_effects_var) {
  ...
  y <- beta[1] * x1 + beta[3] * x3 + random_effect + epsilon
  
  data <- data.frame(y = as.numeric(y), x1 = x1, x2 = x2, x3 = x3, group_id = factor(group_id))
  return(data)
}
...
fit_models <- function(data) {
  models <- list(
    full = lmer(y ~ 0 + x1 + x2 + x3 + (1|group_id), data = data, REML = FALSE),
    reduced1 = lmer(y ~ 0 + x1 + x2 + (1|group_id), data = data, REML = FALSE),
    reduced2 = lmer(y ~ 0 + x1 + x3 + (1|group_id), data = data, REML = FALSE),
    reduced3 = lmer(y ~ 0 + x2 + x3 + (1|group_id), data = data, REML = FALSE)
  )
  return(models)
}
...
simulate_and_fit_models <- function(ni, m, beta, random_effects_var, i) {
  ...
  results_df <- data.frame(
    ...
    bic_fitzmaurice_reduced3 = bic_fitzmaurice_values["reduced3"],
    ...
    bic_normal_reduced2 = bic_normal_values["reduced2"],
     bic_normal_reduced3 = bic_normal_values["reduced3"],
    ...
    bic_hybrid_reduced3 = bic_hybrid_values["reduced3"],
...
bic_columns <- c("bic_fitzmaurice_full", "bic_fitzmaurice_reduced1", "bic_fitzmaurice_reduced2", "bic_fitzmaurice_reduced3",
                 "bic_normal_full", "bic_normal_reduced1", "bic_normal_reduced2", "bic_normal_reduced3",
                 "bic_hybrid_full", "bic_hybrid_reduced1", "bic_hybrid_reduced2", "bic_hybrid_reduced3")

extract_model_name <- function(bic_name) {
  sub("bic_\\w+_", "", bic_name)
}
...
for (name in names(results_list)) {
  ...
  bics_long <- pivot_longer(all_results, cols = all_of(bic_columns), names_to = "BIC_definition_model", values_to = "BIC")
  
  # Extract BIC_definition and model_name
  bics_long <- bics_long %>%
    mutate(BIC_definition = sub("_(full|reduced1|reduced2|reduced3)$", "", BIC_definition_model),
           model_name = extract_model_name(BIC_definition_model))
  ...
  violin_plot <- ggplot(bic_def_example, aes(x = model_name, y = BIC)) +
    geom_violin() +
    theme_minimal() +
...
for (name in names(results_list)) {
  ...
  violin_plot_all <- ggplot(bics_long, aes(x = model_name, y = BIC, fill = model_name)) +
    geom_violin() +
  ...
  violin_plots_all[[name]] <- violin_plot_all
}

# Combine all violin plots for all BIC definitions using patchwork
combined_violin_plot_all <- wrap_plots(violin_plots_all, ncol = 1) & theme(plot.margin = margin(0, 0, 0, 0))
...
selected_models_long <- pivot_longer(all_results, 
  ...
  mutate(BIC_definition = factor(BIC_definition, 
                                 levels = c("selected_model_fitzmaurice", "selected_model_hybrid", "selected_model_normal"), 
                                 labels = c("BIC_f","BIC_h", "BIC_s")))
...
ggplot(selected_models_long, aes(x = BIC_definition, fill = selected_model)) +
    ...
    theme_minimal() +
    labs(title = "Model Selection Percentage",
         x = "", y = "Percentage",
         fill = "Selected Model") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.margin = margin(0, 0, 0, 0)) 
